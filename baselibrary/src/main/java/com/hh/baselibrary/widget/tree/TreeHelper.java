...
public class TreeHelper {
    ...
    private static <T> List<Node> convetData2Node(List<T> datas)
        ...
        for (T t : datas) {
            ...
            for (Field f : declaredFields) {
                ...
                if (f.getAnnotation(TreeNodeSelect.class) != null) {
                    f.setAccessible(true);
                    select = (String) f.get(t);
                }
            }
            node = new Node(id, pId, label, select);
            nodes.add(node);
        }
    ...
    private static void setNodeIcon(Node node) {
        if (node.getChildren().size() > 0 && node.isExpand()) {
            node.setIcon(R.mipmap.retract);
        } else if (node.getChildren().size() > 0 && !node.isExpand()) {
            node.setIcon(R.mipmap.subordinate);
        } else {  //没有子节点（暂时不显示图标）
            node.setIcon(-1);
        }
    }
